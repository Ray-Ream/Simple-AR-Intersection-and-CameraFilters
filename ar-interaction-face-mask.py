...
# 載入更換眼睛圖案的圖片
eye_normal = cv2.imread("Materials/cat_HD.jpg", -1)
        ...
        if results.multi_face_landmarks:
            ...
            for face_landmarks in results.multi_face_landmarks:
                ...
                for index in eye_list:
                    ...
                    if index == 4 or index == 168:
                        eye_size.append([x, y])
                if len(eye_size) == 2:  # 歐幾里得距離
                    eye_len = int(math.pow(math.pow((eye_size[0][0] - eye_size[1][0]), 2) +
                                           math.pow((eye_size[0][1] - eye_size[1][1]), 2), 0.5))
            ...
            try:
                ...
                eye = cv2.resize(eye_normal, (eye_len + 300, eye_len + 300))

                img_height, img_width, _ = eye.shape
                x, y = int(center[0] - img_width / 2), int(center[1] - img_height / 2)
                frame[y: y + img_height, x: x + img_width] = eye
            except:
                pass
